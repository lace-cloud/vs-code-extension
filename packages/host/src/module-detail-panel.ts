...
export async function showModuleDetail(
  ...
  if (transport) {
    ...
    try {
      const getResult = await transport.registryGetVersions({
        name: mod.name,
        system: mod.system,
      });
    ...
    // Fetch module interface (independent)
    try {
      const inspectResult = await transport.inspectModule({
        name: mod.name,
        system: mod.system,
        version: mod.version,
      });

      if (inspectResult?.module_interface) {
        moduleInterface = {
          inputs: inspectResult.module_interface.inputs.map((f) => ({
            name: f.name,
            type: f.type,
            required: f.required,
            description: f.description,
          })),
          outputs: inspectResult.module_interface.outputs.map((f) => ({
            name: f.name,
            type: f.type,
            description: f.description,
          })),
      ...
      const message = err instanceof Error ? err.message : String(err);
      console.warn('Failed to fetch module interface:', message);
      fetchError = 'Could not load module interface. Some details may be unavailable.';
    }
  }
